 ...
 *
 */
public class Message2 extends Workflow {
	public Message2(String instanceid) {
...
//		System.out.println(seconddpType);
		...
		if (firstdpType.equalsIgnoreCase("Dropbox")){
			...
			a = ConversionFunction.fileToScalar(downloadedFilePath, "string").trim();
...
//				b = ((StringDP) inputPortID_to_DP.get("i2")).data;
//		}
		executionStatus = ExecutionStatus.inProcessOfExecution;
		String c = "";
		String resultFileName = instanceId + ".o1." + runID;
		try {
			c = a +", this is Message2";
			if (outputdpType.indexOf("outputDP") >= 0){
				// here convert scalar(boolean) back to dropbox...
				ConversionFunction.scalarToFile(c, Utility.pathToConfigFile.substring(0, Utility.pathToConfigFile.lastIndexOf("/")+1) + resultFileName + ".txt");
